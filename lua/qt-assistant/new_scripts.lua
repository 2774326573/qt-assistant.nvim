...
function M.get_scripts_directory()
	...
	return system.join_path(config.project_root, config.directories.scripts)
end

-- 初始化脚本目录
function M.init_scripts_directory()
	local success, error_msg = M.ensure_scripts_directory()
...
function M.show_script_status()
	...
	for name, script_status in pairs(status.scripts) do
		...
		local status_text = script_status.exists and (script_status.executable and "READY" or "NOT EXECUTABLE")
			or "MISSING"
