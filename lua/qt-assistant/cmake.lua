...
-- Generate third-party library CMake configuration
-- 生成第三方库的CMake配置
function M.generate_third_party_config()
    local config = get_config()
    
    if not config.third_party or not config.third_party.enabled then
        vim.notify("Third-party library management is not enabled", vim.log.levels.WARN)
        return nil
    end
    
    if not config.third_party.libraries or vim.tbl_isempty(config.third_party.libraries) then
        return "# No third-party libraries configured\n"
    end
    ...
    local cmake_lines = {
        ...
        string.format('set(THIRD_PARTY_DIR "${CMAKE_SOURCE_DIR}/%s")', config.third_party.root_dir or "third_party"),
        ""
    }
    
    for lib_name, lib_config in pairs(config.third_party.libraries) do
        table.insert(cmake_lines, string.format("# Configure %s", lib_name))
        
        if lib_config.path then
            local lib_path = lib_config.path
            -- Convert relative path
            if not lib_path:match("^/") and not lib_path:match("^%a:") then
                lib_path = string.format('${CMAKE_SOURCE_DIR}/%s', lib_path)
            end
            
            -- Set library root
            local var_name = lib_name:upper() .. "_ROOT"
            table.insert(cmake_lines, string.format('set(%s "%s")', var_name, lib_path))
            
            -- Add include directories
            if lib_config.include_dir then
                table.insert(cmake_lines, string.format('include_directories("%s/%s")', lib_path, lib_config.include_dir))
            end
            
            -- Add library directories
            if lib_config.lib_dir then
                table.insert(cmake_lines, string.format('link_directories("%s/%s")', lib_path, lib_config.lib_dir))
            end
            
            table.insert(cmake_lines, "")
        end
    end
    
    -- Add find_package calls if auto_cmake is enabled
    if config.third_party.auto_cmake then
        table.insert(cmake_lines, "# Find third-party packages")
        for lib_name, lib_config in pairs(config.third_party.libraries) do
            if lib_config.use_find_package then
                local find_line = string.format("find_package(%s", lib_name)
                if lib_config.version then
                    find_line = find_line .. " " .. lib_config.version
                end
                if lib_config.required then
                    find_line = find_line .. " REQUIRED"
                end
                if lib_config.components then
                    find_line = find_line .. " COMPONENTS " .. table.concat(lib_config.components, " ")
                end
                find_line = find_line .. ")"
                table.insert(cmake_lines, find_line)
            end
        end
        table.insert(cmake_lines, "")
    end
    
    return table.concat(cmake_lines, "\n")
end

-- Add third-party library to CMakeLists.txt
-- 添加第三方库到CMakeLists.txt
function M.add_third_party_library(lib_name, lib_config, start_dir)
    local cmake_file = M.find_cmake_file(start_dir)
    if not cmake_file then
        vim.notify("CMakeLists.txt not found", vim.log.levels.ERROR)
        return false
    ...
    local lib_cmake = string.format([[

# Third-party library: %s
set(%s_ROOT "${CMAKE_SOURCE_DIR}/%s")
]], lib_name, lib_name:upper(), lib_config.path or ("third_party/" .. lib_name))
    
    if lib_config.include_dir then
        lib_cmake = lib_cmake .. string.format('include_directories("${%s_ROOT}/%s")\n', 
            lib_name:upper(), lib_config.include_dir)
    end
    
    if lib_config.lib_dir then
        lib_cmake = lib_cmake .. string.format('link_directories("${%s_ROOT}/%s")\n', 
            lib_name:upper(), lib_config.lib_dir)
    end
    
    if lib_config.use_find_package then
        local find_cmd = string.format("find_package(%s", lib_name)
        if lib_config.version then
            find_cmd = find_cmd .. " " .. lib_config.version
        end
        if lib_config.required then
            find_cmd = find_cmd .. " REQUIRED"
        end
        if lib_config.components then
            find_cmd = find_cmd .. " COMPONENTS " .. table.concat(lib_config.components, " ")
        end
        find_cmd = find_cmd .. ")"
        lib_cmake = lib_cmake .. find_cmd .. "\n"
    end
    
    -- Insert before target definition or at the end
    local insert_pos = content:find("add_executable") or content:find("add_library")
    if insert_pos then
        -- Insert before target definition
        content = content:sub(1, insert_pos - 1) .. lib_cmake .. "\n" .. content:sub(insert_pos)
    else
        -- Append to end
        content = content .. "\n" .. lib_cmake
    end
    ...
    -- Write back
    local success = file_manager.write_file(cmake_file, content)
    if success then
        vim.notify(string.format("Added third-party library '%s' to CMakeLists.txt", lib_name), vim.log.levels.INFO)
    else
...
-- Generate CMake configuration for importing third-party libraries
-- 生成导入第三方库的CMake配置文件
function M.create_third_party_cmake_file(output_path)
    local config = get_config()
    ...
    local cmake_content = M.generate_third_party_config()
    if not cmake_content then
        return false
    end
    
    output_path = output_path or system.join_path(file_manager.get_project_root(), "cmake", "ThirdParty.cmake")
    ...
    local success = file_manager.write_file(output_path, cmake_content)
    if success then
        vim.notify(string.format("Created third-party CMake configuration: %s", output_path), vim.log.levels.INFO)
        
        -- Suggest including it in main CMakeLists.txt
        vim.notify("Add this line to your main CMakeLists.txt: include(cmake/ThirdParty.cmake)", vim.log.levels.INFO)
    else
        vim.notify("Failed to create third-party CMake file", vim.log.levels.ERROR)
    end
