...
function M.format_file(file_path, formatter_name)
    ...
    vim.notify("Formatting " .. vim.fn.fnamemodify(file_path, ":t") .. " with " .. selected_formatter.name .. "...", vim.log.levels.INFO)
...
function M.format_project(formatter_name)
    ...
    for _, source_dir in ipairs(source_dirs) do
        if system.directory_exists(source_dir) then
            ...
            if count then total_formatted = total_formatted + count end
        end
    end
    
    -- 也格式化项目根目录下的源文件
    local root_count = M.format_directory(project_root, formatter_name)
    if root_count then total_formatted = total_formatted + root_count end
    
    if total_formatted == 0 then
        vim.notify("No files were formatted", vim.log.levels.WARN)
    else
        vim.notify(string.format("Project formatting complete! Formatted %d files", total_formatted), vim.log.levels.INFO)
    end
...
function M.create_clang_format_config()
    ...
    if vim.fn.filereadable(config_file) == 1 then
        local overwrite = vim.fn.confirm(
            ".clang-format already exists. Overwrite?",
            "&Yes\n&No", 2
        )
        if overwrite ~= 1 then
            return false
...
function M.setup_auto_format()
    ...
    local available_formatters = M.detect_available_formatters()
    if #available_formatters == 0 then
        vim.notify("No code formatters available. Install clang-format or astyle for auto-formatting.", vim.log.levels.WARN)
        return
    end
    ...
    vim.notify("Qt Assistant auto-formatting enabled for C++ files", vim.log.levels.INFO)
end
